  ...
  onOpen() {
    ...
    if (collision !== null) {
      ...
      new import_obsidian5.Setting(alert).setName("If the file exists").addDropdown(
        (dd) => dd.addOption("overwrite", `Overwrite ${name(collision.existing)}`).addOption("number", `Save as ${name(collision.numbered)}`).setValue(plan.overwrite ? "overwrite" : "number").onChange((v) => {
