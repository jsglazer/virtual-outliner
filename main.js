...
var PdfExporter = class {
  ...
  async plan(file, doc) {
    ...
    return {
      ...
      outputPath: (_e = collision == null ? void 0 : collision.numbered) != null ? _e : target,
      collision,
      overwrite: false,
...
var ExportPdfModal = class extends import_obsidian5.Modal {
  ...
  onOpen() {
    ...
    if (collision !== null) {
      ...
      new import_obsidian5.Setting(alert).setName("If the file exists").addDropdown(
        (dd) => dd.addOption("number", `Save as ${name(collision.numbered)}`).addOption("overwrite", `Overwrite ${name(collision.existing)}`).setValue(plan.overwrite ? "overwrite" : "number").onChange((v) => {
