...
- **An existing PDF is never replaced without asking.** If the PDF is already there, the export dialog (and the Quick Action) says so and defaults to saving the next free numbered copy (`Note-01.pdf`, `Note-02.pdf`, …); choose **Overwrite** to replace it instead.
