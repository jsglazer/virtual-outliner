...
- **Dev or Submit.** The export dialog's **Version** dropdown picks the output template. *Dev* (the default) is the working copy — the preamble exactly as written, date/time stamp in the footer. *Submit* is the hand-in copy: no timestamp, and the page count centred in the footer. Choosing Submit also offers a suffix for the file name — `-Submit` by default, remembered from last time — so the hand-in copy sits beside the working one instead of replacing it. The Finder Quick Action always renders Dev.
...
- **An existing PDF is never replaced silently.** If the PDF is already there, the export dialog (and the Quick Action) says so and defaults to overwriting it; choose **Save as `Note-01.pdf`** to keep the old one and write the next free numbered copy instead (`Note-01.pdf`, `Note-02.pdf`, …).
